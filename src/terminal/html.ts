...
function rowHtml(row: Row): string {
  const inner = `<span class="t-row__label">${escape(row.label)}</span>${row.detail ? `<span class="t-row__detail">${escape(row.detail)}</span>` : ''}`
...
function paneHtml(pane: Pane): string {
  ...
  return `<section class="t-pane${pane.area ? ` t-pane--${pane.area}` : ''}"><h2 class="t-pane__title">${escape(pane.title)}</h2>${note}${linesHtml(pane.lines ?? [])}${rows}${links}${rain}</section>`
}
...
export function lineHtml(line: Line): string {
  ...
  return `<div class="t-line${line.grid ? ' t-grid' : ''}"${vars.length ? ` style="${vars.join(';')}"` : ''}>${body}</div>`
}
